...
export default function ChartComponent({ node, dateRange }: ChartComponentProps) {
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (chartMode === 'realtime') {
                    ...
                    data.forEach((item: any) => {
                        ...
                        const newValue = item.values[0]; // 공유 메모리에서 가져온 실제 값
        ...
        if (chartMode === 'trend') {
            ...
            const initRealtimeData = () => {
                ...
                node.series.forEach((seriesInfo) => {
                    const tagId = seriesInfo.dataID || '';
    ...
    useEffect(() => {
        ...
        // 시리즈 구성
        const seriesConfig: LineSeriesOption[] = node.series
            .filter(seriesInfo => seriesInfo.dataID) // dataID가 있는 것만
            .map((seriesInfo) => {
                const tagId = seriesInfo.dataID || '';
                const item = chartData.find(d => d.tagId === tagId);

                const getLineType = (style?: string): 'solid' | 'dashed' | 'dotted' => {
                ...
                return {
                    ...
                    lineStyle: {
                        ...
                        type: getLineType(node.lineStyle)
                    },
                    itemStyle: {
                        color: seriesInfo.color || '#5470c6'
                    },
                    ...(node.areaOpacity !== undefined && {
                        areaStyle: {
                            opacity: node.areaOpacity
                        }
                    }),
                    ...(node.dataPointSettings && {
                        showSymbol: node.dataPointSettings.showMarker,
                        symbolSize: node.dataPointSettings.markerSize || 6,
                        symbol: node.dataPointSettings.markerShape,
                        label: {
                            show: node.dataPointSettings.showLabel,
                            position: 'top',
                            fontSize: 10
                        }
                    }),
                    label: {
                        show: node.dataPointSettings?.showLabel || false,
                        position: 'top',
                        fontSize: 10,
        ...
        // yAxisMin과 yAxisMax 처리: "auto"이면 undefined로 설정
        const getYAxisValue = (value: any) => {
        ...
        const option: EChartsOption = {
            ...
            xAxis: isRealtime ? {
                ...
                splitLine: {
                    show: node.axisSettings?.showGridLines !== false
                }
            } : {
                name: '시간',
                type: 'time',
            ...
            yAxis: {
                name: '값',
                type: "value",
                min: getYAxisValue(node.axisSettings?.yAxisMin), 
                max: getYAxisValue(node.axisSettings?.yAxisMax), 
                splitLine: {
                    show: node.axisSettings?.showGridLines !== false
                }
            },
