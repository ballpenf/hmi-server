...
export default function GaugeComponent({ node }: GaugeComponentProps) {
  ...
  useEffect(() => {
    ...
    const axisLineColors: [number, string][] =
      node.colorRanges && node.colorRanges.length > 0
        ? node.colorRanges.map(
            (range) => [range.threshold, range.color] as [number, string]
          )
        : [
            [0.3, "#67e0e3"],
            [0.7, "#37a2da"],
    ...
    gaugeInstanceRef.current.setOption(option, {
      notMerge: false,
      lazyUpdate: true,
    });
  }, [
